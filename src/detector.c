...
int saveLog = 1; 	/* [DanielWu] 儲存log */
int check_mistakes = 0;

#if saveLog
	#include <string.h>		/* [DanielWu] add string.h to use function: memset  */
#endif
...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, float thresh, float iou_thresh, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    ...
    pthread_t load_thread = load_data(args);		/* [DanielWu] 讀數據 */
...
void print_detector_detections(FILE **fps, char *image_path, detection *dets, int total, int classes, int w, int h)
{
    int i, j;
    for (i = 0; i < total; ++i) {
...
/* [DanielWu] repeat char number of times for save log */
/*  note: remember free it when not use                */
char* repeat_char(char ch, int times)
{
	char *repeat_ch = malloc(times * sizeof(char));
	memset(repeat_ch, ch, times);
	repeat_ch[times] = '\0';
...
float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net)
{
    ...
    int m = plist->size;		/* [DanielWu] 指向結構體的指標通過箭頭運算子( -> )訪問 */
    int i = 0;
    int t;
	...
	/* -- [DanielWu] add below 7 line to save log in .txt file -- */
	char *prefix = option_find_str(options, "results", "results");
	char buff[1024];
	FILE **fps = 0;
	
	fps = (FILE**) xcalloc(1, sizeof(FILE *));
	snprintf(buff, 1024, "%s/calculate_mAP_log.txt", prefix);
	fps[0] = fopen(buff, "w");
	/* ---------------------------------------------------------- */
	
    time_t start = time(0);
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        fprintf(stderr, "\r%d", i);							/* [DanielWu] stderr: 用於記錄執行時的錯誤或除錯資訊 */
        for (t = 0; t < nthreads && (i + t - nthreads) < m; ++t) {
            pthread_join(thr[t], 0);
            val[t] = buf[t];
            val_resized[t] = buf_resized[t];
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            if (args.type == LETTERBOX_DATA) {
                ...
                dets = get_network_boxes(&net, 1, 1, thresh, hier_thresh, 0, 0, &nboxes, letter_box);  	/* [DanielWu] yolov4 use this */
            }
            //detection *dets = get_network_boxes(&net, val[t].w, val[t].h, thresh, hier_thresh, 0, 1, &nboxes, letter_box); // for letter_box=1
            if (nms) {
                if (l.nms_kind == DEFAULT_NMS) do_nms_sort(dets, nboxes, l.classes, nms);
                else diounms_sort(dets, nboxes, l.classes, nms, l.nms_kind, l.beta_nms);				/* [DanielWu] yolov4 use this */
            }
            ...
            char labelpath[4096];
            replace_image_to_label(path, labelpath);
            int num_labels = 0;											/* [DanielWu] num_labels: 當前影像truth_bbox的數量  */
            box_label *truth = read_boxes(labelpath, &num_labels);
            int j;
            for (j = 0; j < num_labels; ++j) {
			...
			if (saveLog) {												/* [DanielWu] */
				char *h_line = repeat_char('-', 100);
				fprintf(fps[0], "%s\n", h_line);
				fprintf(fps[0], "## Processing: %s\n", path);
				fprintf(fps[0], "   => checkpoint_detections_count: %d\n", checkpoint_detections_count);
				fprintf(fps[0], "   => num_labels: %d\n", num_labels);
				fprintf(fps[0], "   => nboxes: %d\n", nboxes);
				free(h_line);
			}

            int i;
            for (i = 0; i < nboxes; ++i) {								/* [DanielWu] nboxes: 預測bbox的數量  */

                int class_id;
                for (class_id = 0; class_id < classes; ++class_id) {	/* [DanielWu] class_id: 類別id */
                    float prob = dets[i].prob[class_id];				/* [DanielWu] 預測的bbox會依序算出每個類別的機率 (i: 第i個預測的bbox) */
					
					if (saveLog) fprintf(fps[0], "### prob: %f  (dets[%d].prob[%d])\n", prob, i, class_id);	/* [DanielWu] prob: 概率 */
					
					if (prob > 0) {
                        detections_count++;
                        detections = (box_prob*)xrealloc(detections, detections_count * sizeof(box_prob));	/* [DanielWu] realloc: 改變已配置的記憶體大小 */
						...
						if (saveLog) {
							fprintf(fps[0], "\t=> detections_count++: %d\n", detections_count);				/* [DanielWu] 只要機率大於0, detections_count都會+1 */
							fprintf(fps[0], "\t=> bbox: %f, image_index: %d, class_id: %d\n", dets[i].bbox, image_index, class_id);
						}
                        ...
                        for (j = 0; j < num_labels; ++j)
                        {
                            ...
                            float current_iou = box_iou(dets[i].bbox, t);					/* [DanielWu] 偵測到的bbox會與每個truth_bbox做IoU的計算 */
							
                            if (current_iou > iou_thresh && class_id == truth[j].id) {		/* [DanielWu] 偵測到的bbox的IoU大於閥值且預測的標籤等於真實的標籤 */
                                if (current_iou > max_iou) {								/* [DanielWu] 存取最大IoU的index */
                                    truth_index = unique_truth_count + j;

									if (saveLog) {											/* [DanielWu] */
										fprintf(fps[0], "\t    => current_iou = %f > max_iou = %f && class_id = truth_id = %d\n", current_iou, max_iou, class_id);
										fprintf(fps[0], "\t       # dets[%5d].bbox : (%f, %f, %f, %f)\n", i, dets[i].bbox.x, dets[i].bbox.y, dets[i].bbox.w, dets[i].bbox.h);
										fprintf(fps[0], "\t       # truth_bbox\t  : (%f, %f, %f, %f)\n", t.x, t.y, t.w, t.h);
										fprintf(fps[0], "\t       # max_iou\t  : %f\n", max_iou);
										fprintf(fps[0], "\t       # truth_index\t  : %8d", truth_index);
										fprintf(fps[0], "\t(unique_truth_count: %d + j: %d)\n", unique_truth_count, j);
									}
									
									max_iou = current_iou;
                                }
								else {
									/* [DanielWu] 就算current_iou < max_iou, truth_index可能為非-1的整數, truth_index會繼承上一個有符合current_iou > max_iou時,所改變的truth_index   */
									if (saveLog) {
										fprintf(fps[0], "\t    => current_iou = %f <= max_iou = %f\n", current_iou, max_iou);
										fprintf(fps[0], "\t       # current_iou <= iou_thresh && class_id = truth_id\n");
										fprintf(fps[0], "\t       # truth_index\t  : %8d\n", truth_index);
									}
								}
                            }
							else {
								if (saveLog) {
									/* [DanielWu] 就算current_iou < max_iou, truth_index可能為非-1的整數, truth_index會繼承上一個有符合current_iou > max_iou時,所改變的truth_index   */
									fprintf(fps[0], "\t    => current_iou = %f, iou_thresh = %f", current_iou, iou_thresh);
									fprintf(fps[0], ", class_id = %d, truth_id = %d\n", class_id, truth[j].id);
									if (current_iou <= iou_thresh && class_id == truth[j].id) fprintf(fps[0], "\t       # IoU less than the thresh");
									if (current_iou > iou_thresh && class_id != truth[j].id) fprintf(fps[0], "\t       # id is not match");
									if (current_iou <= iou_thresh && class_id != truth[j].id) fprintf(fps[0], "\t       # IoU is less than the thresh and id is not match");
									fprintf(fps[0], "\n\t       # truth_index\t  : %8d\n", truth_index);
								}
							}
                        }
                        ...
                        if (truth_index > -1) {
							...
							if (saveLog) {																/* [DanielWu] */
								char *h_line = repeat_char('-', 50);
								fprintf(fps[0], "\t%s\n", h_line);
								fprintf(fps[0], "\t## best IoU\n");
								fprintf(fps[0], "\t=> truth_index = %d > -1\n", truth_index);
								fprintf(fps[0], "\t    => update detections[%5d].truth_flag\t   = 1\n", detections_count - 1);
								fprintf(fps[0], "\t\t      detections[%5d].unique_truth_index = %d\n", detections_count - 1, truth_index);
								free(h_line);
							}
                        }
                        else {
                            ...
                            for (j = 0; j < num_labels_dif; ++j) {
                                ...
                                if (current_iou > iou_thresh && class_id == truth_dif[j].id) {
									...
									if (saveLog) {														/* [DanielWu] */
										...
										fprintf(fps[0], "\t%s\n", h_line);
										fprintf(fps[0], "\t## best IoU\n");
										fprintf(fps[0], "\t=> truth_index = %d\n", truth_index);		/* [DanielWu] truth_index = -1*/
										fprintf(fps[0], "\t    => current_iou = %f > max_iou = %f && class_id = truth_id = %d\n", current_iou, max_iou, class_id);
										fprintf(fps[0], "\t    => --detections_count = %d\n", detections_count);
										free(h_line);
									}
                        ...
                        if (prob > thresh_calc_avg_iou) {
							...
							if (saveLog) {																/* [DanielWu] */
								char *h_line = repeat_char('-', 50);
								fprintf(fps[0], "\t%s\n", h_line);
								fprintf(fps[0], "\t## calc avg IoU, true-positives, false-positives for required Threshold\n");
								fprintf(fps[0], "\t=> prob > thresh_calc_avg_iou = %f\n", thresh_calc_avg_iou);
								fprintf(fps[0], "\t   (checkpoint_detections_count = %d, detections_count - 1 = %d)\n", checkpoint_detections_count, detections_count - 1);
								free(h_line);
							}
							...
							for (z = checkpoint_detections_count; z < detections_count - 1; ++z) {
                                if (detections[z].unique_truth_index == truth_index) {
                                    found = 1;
									if (saveLog) {														/* [DanielWu] */
										fprintf(fps[0], "\t    => z = %5d, detections[z].unique_truth_index = truth_index = %5d\n", z, truth_index);
										fprintf(fps[0], "\t\t=> found = %d\n", found);
									}
									break;
                                }
								else {
									if (saveLog) {														/* [DanielWu] */
										fprintf(fps[0], "\t    => z = %5d, detections[z].unique_truth_index = %5d, ", z, detections[z].unique_truth_index);
										fprintf(fps[0], "truth_index = %5d\n", truth_index);
										fprintf(fps[0], "\t\t=> found = %d\n", found);
									}
								}
                            }
                            ...
                            if (truth_index > -1 && found == 0) {
								...
								if (saveLog) {															/* [DanielWu] */
									fprintf(fps[0], "\n\t    => truth_index = %5d > -1 && found = %d\n", truth_index, found);
									fprintf(fps[0], "\t    => ++tp_for_thresh = %d\n", tp_for_thresh);
									fprintf(fps[0], "\t       # dets[%5d].bbox : (%f, %f, %f, %f)\n", i, dets[i].bbox.x, dets[i].bbox.y, dets[i].bbox.w, dets[i].bbox.h);
								}
                            }
                            else{
								...
								if (saveLog) {															/* [DanielWu] */
									fprintf(fps[0], "\n\t    => truth_index = %5d, found = %d\n", truth_index, found);
									fprintf(fps[0], "\t    => fp_for_thresh++ = %d\n", fp_for_thresh);
									fprintf(fps[0], "\t       # dets[%3d].bbox : (%f, %f, %f, %f)\n", i, dets[i].bbox.x, dets[i].bbox.y, dets[i].bbox.w, dets[i].bbox.h);
								}
                            }
							...
							if (saveLog) {																/* [DanielWu] */
								...
								fprintf(fps[0], "\t%s\n", h_line);
								for (k = 0; k < classes; k++) {
									fprintf(fps[0], "\ttp_for_thresh_per_class[%d] = %d\n", k, tp_for_thresh_per_class[k]);
									fprintf(fps[0], "\tfp_for_thresh_per_class[%d] = %d\n", k, fp_for_thresh_per_class[k]);
								}
								free(h_line);
							}
                        }
            ...
            unique_truth_count += num_labels;		/* [DanielWu] unique_truth_count: 全部資料集truth_bbox的數量  */
			fprintf(fps[0], "\n   => unique_truth_count = %d\n", unique_truth_count);		/* [DanielWu] */
	...
	if (fps) {						/* [DanielWu] close the .txt file */
		fclose(fps[0]);
		free(fps);
	}
